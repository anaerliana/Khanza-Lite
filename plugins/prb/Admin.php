...
class Admin extends AdminModule
{
  ...
  {
    $sub_modules = [
      ['name' => 'Index', 'url' => url([ADMIN, 'prb', 'index']), 'icon' => 'code', 'desc' => 'Index PRB'],
      ['name' => 'Pengaturan', 'url' => url([ADMIN, 'prb', 'settings']), 'icon' => 'code', 'desc' => 'Pengaturan PRB']
    ];
  ...
  public function anyIndex($page = 1)
  {
    if (isset($_POST['submit'])) {
      if (!$this->db('mlite_srb')->where('nosep', $_POST['nosep'])->oneArray()) {
        $simpan_status = $this->db('mlite_srb')->save([
          'id' => NULL,
          'tanggal' => date('Y-m-d'),
        ...
        $simpan_status = $this->db('mlite_srb')
      ...
      if ($simpan_status) {
        $this->db('mlite_srb_feedback')->save([
          'id' => NULL,
          'nosep' => $_POST['nosep'],
          'tanggal' => date('Y-m-d'),
    ...
    // pagination
    $totalRecords = $this->db()->pdo()->prepare("SELECT reg_periksa.no_rawat FROM reg_periksa, pasien, mlite_srb WHERE reg_periksa.no_rkm_medis = pasien.no_rkm_medis AND reg_periksa.no_rawat = mlite_srb.no_rawat AND (reg_periksa.no_rkm_medis LIKE ? OR reg_periksa.no_rawat LIKE ? OR pasien.nm_pasien LIKE ?) AND reg_periksa.tgl_registrasi BETWEEN '$start_date' AND '$end_date' AND reg_periksa.status_lanjut = 'Ralan'");
    ...
    $query = $this->db()->pdo()->prepare("SELECT reg_periksa.*, pasien.*, dokter.nm_dokter, poliklinik.nm_poli, mlite_srb.no_rawat, mlite_srb.nosep FROM reg_periksa, pasien, dokter, poliklinik, mlite_srb WHERE reg_periksa.no_rkm_medis = pasien.no_rkm_medis AND reg_periksa.kd_dokter = dokter.kd_dokter AND reg_periksa.kd_poli = poliklinik.kd_poli AND reg_periksa.no_rawat = mlite_srb.no_rawat AND (reg_periksa.no_rkm_medis LIKE ? OR reg_periksa.no_rawat LIKE ? OR pasien.nm_pasien LIKE ?) AND reg_periksa.tgl_registrasi BETWEEN '$start_date' AND '$end_date' AND reg_periksa.status_lanjut = 'Ralan' LIMIT $perpage OFFSET $offset");
    ...
    if (count($rows)) {
      foreach ($rows as $row) {
        ...
        $row['pdfURL'] = url([ADMIN, 'prb', 'pdf', $this->convertNorawat($row['no_rawat'])]);
        $row['batalURL'] = url([ADMIN, 'prb', 'batal', $this->convertNorawat($row['no_rawat'])]);
        $row['berkas_digital'] = $berkas_digital;
        $row['berkas_digital_pasien'] = $berkas_digital_pasien;
        $row['formSepURL'] = url([ADMIN, 'prb', 'formsepvclaim', '?no_rawat=' . $row['no_rawat']]);
        $row['setstatusURL']  = url([ADMIN, 'prb', 'setstatus', $this->_getSEPInfo('no_sep', $row['no_rawat'])]);
        $row['status_pengajuan'] = $this->db('mlite_srb')->where('nosep', $this->_getSEPInfo('no_sep', $row['no_rawat']))->desc('id')->limit(1)->toArray();
        $row['berkasPasien'] = url([ADMIN, 'prb', 'berkaspasien', $this->core->getRegPeriksaInfo('no_rkm_medis', $row['no_rawat'])]);
        $row['berkasPerawatan'] = url([ADMIN, 'prb', 'berkasperawatan', $this->convertNorawat($row['no_rawat'])]);
    ...
    $this->assign['searchUrl'] =  url([ADMIN, 'prb', 'index', $page . '?s=' . $phrase . '&start_date=' . $start_date . '&end_date=' . $end_date]);
    return $this->draw('index.html', ['prb' => $this->assign]);
  }
  ...
  {
    $delete = $this->db('mlite_srb')->where('no_rawat', $this->revertNorawat($id))->delete();
  ...
  public function getPDF($id)
  {
    ...
    $this->tpl->set('billing_mlite_srb', htmlspecialchars_array($this->settings('veronisa')));
    ...
    */
    ...
    $list_riwayat_obat = $this->db('reg_periksa')
      ->join('resep_obat', 'resep_obat.no_rawat=reg_periksa.no_rawat')
      ->join('dokter', 'dokter.kd_dokter=resep_obat.kd_dokter')
      ->where('no_rkm_medis',$this->core->getRegPeriksaInfo('no_rkm_medis', $this->revertNorawat($id)))
      ->where('reg_periksa.tgl_registrasi', '>=',  $today2)
      ->where('reg_periksa.tgl_registrasi', '<=', $today)
      ->desc('resep_obat.tgl_perawatan')
      ->toArray();

    foreach($list_riwayat_obat as $list_riw){
    $list_obatpasien = $this->db('detail_pemberian_obat')
            ->join('databarang', 'databarang.kode_brng = detail_pemberian_obat.kode_brng')
            ->where('detail_pemberian_obat.no_rawat', $list_riw['no_rawat'])
            ->where('detail_pemberian_obat.tgl_perawatan', $list_riw['tgl_perawatan'])
            ->where('detail_pemberian_obat.jam', $list_riw['jam'])
            ->toArray();

        $detail_obat_pasien = [];
        foreach ($list_obatpasien as $valuedetail) {
          $valuedetail['aturan_pakai'] = $this->db('aturan_pakai')
             ...
             $detail_obat_pasien[] = [
                    'kode_brng' => $valuedetail['kode_brng'],
        ...
        $list_riw['detail_obat_pasien'] = $detail_obat_pasien;
        $riwayat_obatpasien[] = $list_riw;
    }
    $this->tpl->set('riwayat_obatpasien', $riwayat_obatpasien);


    // cek obat obat_kronis
    $today = date('Y-m-d');
    $today2 = date('Y-m-d', strtotime('-2 years'));

    $cek_pasienkronis = $this->db('mlite_srb')
      ->select('mlite_srb.no_rawat')
      ->join('reg_periksa', 'reg_periksa.no_rawat = mlite_srb.no_rawat')
      ->where('reg_periksa.no_rkm_medis', $this->core->getRegPeriksaInfo('no_rkm_medis', $this->revertNorawat($id)))
      ->where('reg_periksa.tgl_registrasi', '>=',  $today2)
      ->where('reg_periksa.tgl_registrasi', '<=', $today)
      ->toArray();
    $riwayat_obat_kronis = [];
    foreach ($cek_pasienkronis as $value) {
      $list_riwayat_obat = $this->db('resep_obat')
        ->join('dokter', 'dokter.kd_dokter = resep_obat.kd_dokter')
        ->where('resep_obat.no_rawat', $value['no_rawat'])
        ->toArray();

      foreach ($list_riwayat_obat as $list_riw) {
        $list_obat_kronis = $this->db('detail_pemberian_obat')
          ->join('databarang', 'databarang.kode_brng = detail_pemberian_obat.kode_brng')
          ->where('detail_pemberian_obat.no_rawat', $list_riw['no_rawat'])
          ->where('detail_pemberian_obat.tgl_perawatan', $list_riw['tgl_perawatan'])
          ->where('detail_pemberian_obat.jam', $list_riw['jam'])
        ...
        $detail_obat_kronis = [];
        foreach ($list_obat_kronis as $valuedetail) {
          $valuedetail['aturan_pakai'] = $this->db('aturan_pakai')
            ->where('aturan_pakai.no_rawat', $valuedetail['no_rawat'])
            ->where('aturan_pakai.tgl_perawatan', $valuedetail['tgl_perawatan'])
            ->where('aturan_pakai.jam', $valuedetail['jam'])
        ...
        $list_riw['detail_obat_kronis'] = $detail_obat_kronis;
        $riwayat_obat_kronis[] = $list_riw;
      }
    }

    $this->tpl->set('riwayat_obat_kronis', $riwayat_obat_kronis);
  ...
  public function getSetStatus($id)
  {
    ...
    // $veronisa = $this->db('mlite_srb')->join('mlite_srb_feedback','mlite_srb_feedback.nosep=mlite_srb.nosep')->where('status','<>','')->where('mlite_srb.nosep', $id)->asc('mlite_srb.id')->toArray();
    $rows_veronisa = $this->db('mlite_srb')->join('mlite_srb_feedback','mlite_srb_feedback.nosep=mlite_srb.nosep')->where('status','<>','')->where('mlite_srb.nosep', $id)->asc('mlite_srb.id')->toArray();
  ...
  {
    $this->_addHeaderFiles();
    $this->assign['title'] = 'Pengaturan Modul PRB';
    $this->assign['prb'] = htmlspecialchars_array($this->settings('prb'));
    $this->assign['master_berkas_digital'] = $this->db('master_berkas_digital')->toArray();
