...
return [
    ...
    'install'       =>  function () use ($core) {
        ...
        $core->db()->pdo()->exec("CREATE TABLE IF NOT EXISTS `mlite_users_vedika` (
          `id` int(11) NOT NULL,
          `username` text,
          `password` text,
          `fullname` text
        ...
        $core->db()->pdo()->exec("ALTER TABLE `mlite_users_vedika`
          ADD PRIMARY KEY (`id`);");

        $core->db()->pdo()->exec("ALTER TABLE `mlite_users_vedika`
          MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;");

        $core->db()->pdo()->exec("CREATE TABLE IF NOT EXISTS `mlite_purif` (
          `id` int(11) NOT NULL,
        ...
        $core->db()->pdo()->exec("ALTER TABLE `mlite_purif`
        ADD PRIMARY KEY (`id`);");
