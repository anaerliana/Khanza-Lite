...
class Admin extends AdminModule
{
  ...
  public function getEncounterBundle($no_rawat)
  {
    ...
    $tgl_registrasi = $this->core->getRegPeriksaInfo('tgl_registrasi', $no_rawat);
    $jam_reg = $this->core->getRegPeriksaInfo('jam_reg', $no_rawat);
    $inProg = $this->db('pemeriksaan_ralan')->select(['tgl' => 'tgl_perawatan', 'jam' => 'jam_rawat'])->where('no_rawat', $no_rawat)->oneArray();
  ...
  {
    $poli = $this->db('dokter')->where('status', '1')->toArray();
    $mlite_satset = $this->db('mlite_satu_sehat_mapping_praktisi')->toArray();
    $satu_sehat = [];
    foreach ($mlite_satset as $value) {
    ...
    return $this->draw('map_praktisi.html', ['departemen' => $this->db('departemen')->toArray(), 'poli' => $poli, 'satu_sehat_departemen' => $satu_sehat]);
  }
  ...
  public function getResponse()
  {
    ...
    foreach ($query as $row) {
      ...
      $mlite_billing = $this->db('billing')->where('no_rawat', $row['no_rawat'])->oneArray();
