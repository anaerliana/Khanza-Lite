...
class Admin extends AdminModule
{
  ...
  public function getManage()
  {
    $sub_modules = [
      ...
      ['name' => 'Pengajuan', 'url' => url([ADMIN, 'veronisa', 'pengajuan']), 'icon' => 'code', 'desc' => 'Index Pengajuan Veronisa'],
      ['name' => 'Perbaikan', 'url' => url([ADMIN, 'veronisa', 'perbaikan']), 'icon' => 'code', 'desc' => 'Index Perbaikan Veronisa'],
  ...
  public function getPDF($id)
  {
    ...
    */
    ...
    $cek_pasienkronis = $this->db('mlite_veronisa')
      ->select('mlite_veronisa.no_rawat')
      ->join('reg_periksa', 'reg_periksa.no_rawat = mlite_veronisa.no_rawat')
   ...
   public function anyPengajuan($page = 1)
  {
    ...
    // pagination
    $totalRecords = $this->db()->pdo()->prepare("SELECT reg_periksa.no_rawat FROM reg_periksa, pasien, mlite_veronisa WHERE reg_periksa.no_rkm_medis = pasien.no_rkm_medis AND reg_periksa.no_rawat = mlite_veronisa.no_rawat AND mlite_veronisa.status = 'Pengajuan' AND (reg_periksa.no_rkm_medis LIKE ? OR reg_periksa.no_rawat LIKE ? OR pasien.nm_pasien LIKE ?) AND reg_periksa.tgl_registrasi BETWEEN '$start_date' AND '$end_date' AND reg_periksa.status_lanjut = 'Ralan'");
    $totalRecords->execute(['%' . $phrase . '%', '%' . $phrase . '%', '%' . $phrase . '%']);
    $totalRecords = $totalRecords->fetchAll();
    ...
    $offset = $pagination->offset();
    $query = $this->db()->pdo()->prepare("SELECT * FROM mlite_veronisa WHERE status = 'Pengajuan' AND (no_rkm_medis LIKE ? OR no_rawat LIKE ? OR nosep LIKE ?) AND tgl_registrasi BETWEEN '$start_date' AND '$end_date' ORDER BY nosep LIMIT $perpage OFFSET $offset");
    $query->execute(['%' . $phrase . '%', '%' . $phrase . '%', '%' . $phrase . '%']);
    $rows = $query->fetchAll();
    ...
    if (count($rows)) {
      foreach ($rows as $row) {
        ...
        $row['status_pengajuan'] = $this->db('mlite_veronisa')->where('nosep', $this->_getSEPInfo('no_sep', $row['no_rawat']))->desc('id')->limit(1)->toArray();
        $row['berkasPerawatan'] = url([ADMIN, 'veronisa', 'berkasperawatan', $this->convertNorawat($row['no_rawat'])]);
        $row['pegawai'] = $this->db('mlite_veronisa_feedback')->join('pegawai','pegawai.nik=mlite_veronisa_feedback.username')->where('nosep', $this->_getSEPInfo('no_sep', $row['no_rawat']))->desc('mlite_veronisa_feedback.id')->limit(1)->toArray();
        $this->assign['list'][] = $row;
      }
    }
   ...
   public function anyPerbaikan($page = 1)
  {
    if (isset($_POST['submit'])) {
      if (!$this->db('mlite_veronisa')->where('nosep', $_POST['nosep'])->oneArray()) {
        $simpan_status = $this->db('mlite_veronisa')->save([
    ...
    // pagination
    $totalRecords = $this->db()->pdo()->prepare("SELECT no_rawat FROM mlite_veronisa WHERE status = 'Perbaiki'  AND (no_rkm_medis LIKE ? OR no_rawat LIKE ? OR nosep LIKE ?) AND tgl_registrasi BETWEEN '$start_date' AND '$end_date'");
    $totalRecords->execute(['%' . $phrase . '%', '%' . $phrase . '%', '%' . $phrase . '%']);
    $totalRecords = $totalRecords->fetchAll();

    $pagination = new \Systems\Lib\Pagination($page, count($totalRecords), $perpage, url([ADMIN, 'veronisa', 'perbaikan', '%d?s=' . $phrase . '&start_date=' . $start_date . '&end_date=' . $end_date]));
    $this->assign['pagination'] = $pagination->nav('pagination', '5');
    $this->assign['totalRecords'] = $totalRecords;

    $offset = $pagination->offset();
    $query = $this->db()->pdo()->prepare("SELECT * FROM mlite_veronisa WHERE status = 'Perbaiki' AND (no_rkm_medis LIKE ? OR no_rawat LIKE ? OR nosep LIKE ?) AND tgl_registrasi BETWEEN '$start_date' AND '$end_date'  ORDER BY nosep LIMIT $perpage OFFSET $offset");
    $query->execute(['%' . $phrase . '%', '%' . $phrase . '%', '%' . $phrase . '%']);
    $rows = $query->fetchAll();
    ...
    $this->assign['searchUrl'] =  url([ADMIN, 'veronisa', 'perbaikan', $page . '?s=' . $phrase . '&start_date=' . $start_date . '&end_date=' . $end_date]);
    return $this->draw('perbaikan.html', ['veronisa' => $this->assign]);
  }
  ...
  public function getPengajuanExcel()
  {
    $start_date = $_GET['start_date'];
    $end_date = $_GET['end_date'];
    $rows = $this->db('mlite_veronisa')->where('status', 'Pengajuan')->where('tgl_registrasi','>=',$start_date)->where('tgl_registrasi','<=', $end_date)->toArray();
    $i = 1;
    foreach ($rows as $row) {
      ...
      $get_feedback_bpjs = $this->db('mlite_veronisa_feedback')->where('nosep', $row['nosep'])->where('username', 'bpjs')->oneArray();
      $row['konfirmasi_bpjs'] = $get_feedback_bpjs['catatan'];
      $get_feedback_rs = $this->db('mlite_veronisa_feedback')->where('nosep', $row['nosep'])->where('username','!=','bpjs')->oneArray();
    ...
    echo $this->tpl->draw(MODULES . '/veronisa/view/admin/pengajuan_excel.html', true);
    exit();
  }
   ...
   public function getPerbaikanExcel()
  {
    $start_date = $_GET['start_date'];
    $end_date = $_GET['end_date'];
    $rows = $this->db('mlite_veronisa')->where('status', 'Perbaiki')->where('tgl_registrasi','>=',$start_date)->where('tgl_registrasi','<=', $end_date)->toArray();
    $i = 1;
    foreach ($rows as $row) {
      $row['status_lanjut'] = 'Ralan';
      $row['no'] = $i++;
    ...
    echo $this->tpl->draw(MODULES . '/veronisa/view/admin/perbaikan_excel.html', true);
    exit();
  }
